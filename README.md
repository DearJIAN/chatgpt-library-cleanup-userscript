...
如果用户希望“按照 ChatGPT Library 页面上实际看到的「修改时间」清理旧文件”，那么截止日期判断应当以 `updated_at` 为主，而不能把 `record_creation_time` 当作 UI「修改时间」的等价字段。

需要区分两个不同概念：

- **创建 / 首次进入 Library 的时间**：更接近 `record_creation_time` / `file_upload_time`；
- **Library UI 的「修改时间」**：已实测确认对应 `updated_at`，文件重命名等修改会推进该时间。

> **版本说明：截至 0.8.7，以上字段映射已经实测确认，但真实删除日期语义仍保持此前的 `record_creation_time → file_upload_time → created/upload fallback`，尚未因诊断结论自动切换。后续代码版本若将截止日期语义与 UI「修改时间」对齐，应以 `updated_at` 为主，并同步更新本节及使用说明。**

这也是“诊断时间字段”功能坚持只输出事实、不自动改变删除规则的原因：在证据不足时宁可保持原行为，只有完成可重复验证后再调整实际删除语义。
...
也就是说，截至 0.8.7，当前截止日期判断仍针对**每一个文件自己的创建 / 上传时间**，并不等同于 Library UI 的「修改时间」。UI「修改时间」已通过可控实验确认对应 `updated_at`；详细证据见上文 [Library「修改时间」字段实测结论](#library修改时间字段实测结论)。
