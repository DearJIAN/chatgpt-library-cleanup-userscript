...
- 当前 session 首次 probe 成功后会暂停，并要求确认测试文件已从 Library 主列表消失；选择继续后记住 session 验证状态，后续验证轮次和再次流式清理不再重复询问；选择停止则不发送第二个删除请求；
- “删除已扫描旧文件”允许在扫描被停止后处理当前 `state.scan.records`，明确提示未扫描部分不会处理，不启动全库 verification；截止日期含当天，实际比较使用所选日期次日 00:00 的 exclusive end；
